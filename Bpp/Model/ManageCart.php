...
//                    ],
     ...
     * @throws \Exception
     */
    public function removeQuoteItem(CartInterface $quote, array $allItems)
    {
        $removeItem = [];
        ...
        if (!empty($removeItem)) {
            $availableItem = $quote->getAllVisibleItems();
            /**
             * @var \Magento\Quote\Model\Quote\Item $eachItem
             */
            foreach ($availableItem as $eachItem) {
                if (in_array($eachItem->getSku(), $removeItem)) {
                    $quote->deleteItem($eachItem);
